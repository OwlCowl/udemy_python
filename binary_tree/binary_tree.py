...
class BinaryTree:
    ...
    def add(self, new_node: Node):
        ...
        while current_node:
            if current_node.value == new_node.value:
                raise ValueError('This value already exsists')
    ...
    def find(self, value:int):
        ...
        while current_node:
            if current_node.value == value:
                ...
                current_node = current_node.right
        raise LookupError('There is no such value')
    ...
    def find_parent(self, value: int) -> Node:
        ...
        while current_node:
            if (current_node.left and current_node.left.value == value) or\
                    (current_node.right and current_node.right == value):
                return current_node
    ...
    def delete(self, value: int):
        to_delete = self.delete(value)
        to_delete_parent = self.find_parent(value)

        """We have 2 children, need to check if node left of right"""
        if to_delete.left and to_delete.right:
            rightmost = self.find_rightmost(to_delete.left)
            rightmost_parent = self.find_parent(value)
